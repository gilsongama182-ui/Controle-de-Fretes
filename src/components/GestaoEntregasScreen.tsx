...
import { 
  ...
  Search, Download, PlusCircle, CheckCircle, AlertTriangle, Clock, 
...
export default function GestaoEntregasScreen({
  ...
  const handleDelete = (id: string, nfe: string) => {
    if (confirm(`Tem certeza que deseja remover a entrega NF-e ${nfe} (ID ${id})?`)) {
      onDeleteDelivery(id);
  ...
  const handleSimulateImport = () => {
    ...
    const mockDel: Delivery = {
      ...
      valorPagamento: 1350.00,
      codigoRastreio: `HB2024TX${randomNum}`
    };
    onAddDelivery(mockDel);
    alert('Planilha importada! 1 nova entrega inserida no banco de dados local.');
  };
    ...
    <div className="bg-surface text-on-surface font-sans min-h-screen flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-[280px] bg-surface-container-low border-r border-outline-variant flex flex-col py-6 px-4 shrink-0 md:sticky md:top-0 md:h-screen">
        ...
        <div className="mt-auto pt-4 border-t border-outline-variant">
          <button
            onClick={() => onSelectDeliveryForEdit(deliveries[0])}
            className="w-full mb-4 flex items-center justify-center gap-2 bg-primary text-on-primary py-3 px-4 rounded-xl font-bold hover:opacity-90 transition-all shadow-md text-sm"
          >
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="relative w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 border border-outline-variant rounded-lg text-sm bg-white focus:ring-2 focus:ring-primary outline-none"
                />
              </div>
